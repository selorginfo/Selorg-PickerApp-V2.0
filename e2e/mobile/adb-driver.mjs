...
const PACKAGE = process.env.ANDROID_PACKAGE || "com.selorgpickerapp";
...
export function ensureMetroHostPrefs() {
  ...
  // Force localhost so adb reverse works (avoids Android 17 local-network / 10.0.2.2 issues).
  const xml = `<?xml version='1.0' encoding='utf-8' standalone='yes' ?>
<map>
    <string name="debug_http_host">localhost:8081</string>
</map>
`;
  const local = path.join(ARTIFACTS, "com.selorgpickerapp_preferences.xml");
  try {
    ...
    adb(
      [
        ...
        "shared_prefs/com.selorgpickerapp_preferences.xml",
      ],
      { timeout: 15_000 },
    );
  } catch {
    /* first launch may lack prefs dir — retry after start */
  }
  try {
...
export function launchApp(clearData = false) {
  if (clearData) {
    ...
    ensureReversePorts();
    // Seed prefs dir, stamp localhost Metro host, then cold-start for real.
    adb([
      "shell",
      "am",
      "start",
...
export function readAppSessionToken() {
  // AsyncStorage key @selorg-picker/token → JSON-encoded JWT string in RKStorage.
    ...
    try {
      const out = adb(
        [
          ...
          "sh",
          "-c",
          `sqlite3 ${dbPath} "SELECT value FROM catalystLocalStorage WHERE key='@selorg-picker/token' LIMIT 1;" 2>/dev/null || true`,
        ],
        { timeout: 12_000 },
      );
  ...
  if (!raw) {
    ...
    try {
      ...
      const line = String(dump || "")
        .split(/\r?\n/)
        .find((l) => /@selorg-picker\/token|token/.test(l));
      if (line) {
